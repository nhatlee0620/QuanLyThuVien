...
namespace PhanMemQLTV
{
    public partial class frmBaoCaoThongKe : Form
    {
        ...
        private void slMuon()
        {
            ...
            {
                MessageBox.Show("Chưa có độc giả nào mượn sách");
            }
        ...
        private void btnXemSLSachQuaHan_Click(object sender, EventArgs e)
        {
            ...
            dataGridViewDSDGQuaHan.Columns["Mã Phiếu"].Width = 100;
            dataGridViewDSDGQuaHan.Columns["Mã DG"].Width = 100;
            dataGridViewDSDGQuaHan.Columns["Mã Sách"].Width = 100;
            dataGridViewDSDGQuaHan.Columns["Ngày Mượn"].Width = 150;
            dataGridViewDSDGQuaHan.Columns["Ngày Trả"].Width = 150;
            dataGridViewDSDGQuaHan.Columns["Ghi Chú"].Width = 140;
        ...
        {
            string strTimSLDGQuaHan = @"SELECT MaDG as 'Mã DG', sum(SLMuon) as 'Số Lượng Sách Quá Hạn' from tblHSPhieuMuon where CONVERT (datetime, NgayTra, 103) <= getdate() group by MaDG";
            dataGridViewDSDGQuaHan.DataSource = ketnoi(strTimSLDGQuaHan);
            dataGridViewDSDGQuaHan.Columns["Mã DG"].Width = 300;
            dataGridViewDSDGQuaHan.Columns["Số Lượng Sách Quá Hạn"].Width = 440;
            dataGridViewDSDGQuaHan.SelectionMode=DataGridViewSelectionMode.FullRowSelect;
            dataGridViewDSDGQuaHan.Show();
        }
